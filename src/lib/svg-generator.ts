...
export function generateMossPole(data: MossPolesData): string {
  ...
  const defaultVisual = defaultConfig.config.visual;

  const polesPerRow = config.visual.polesPerRow || defaultVisual.polesPerRow;
  ...
  const backgroundColor = config.visual.backgroundColor || defaultVisual.backgroundColor;
  const textColor = config.visual.textColor || defaultVisual.textColor;
  const algaePercentage: number = config.visual.algaePercentage ?? defaultVisual.algaePercentage;
  ...
  const tips = config.careTips.tips || defaultConfig.config.careTips.tips;
  const wrappedTips: string[][] = (tips || []).map(tip => wrapText(tip, instructionsWidth - 40));

  const titleHeight = 40;
  const tipsHeight = wrappedTips.reduce((acc, tip) => acc + (tip.length * 20) + 10, 0);
    ...
    <svg viewBox="0 0 ${baseWidth} ${totalHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      ${config.title.enabled ? `
        <text x="${baseWidth / 2}" y="${topPadding}" text-anchor="middle" font-size="24" fill="${textColor}">
          ${config.title.text || defaultConfig.config.title.text}
        </text>
      ` : ''}
