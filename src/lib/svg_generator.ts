...
export function generateMossPole(data: MossPolesData): string {
    ...
    const nameTextY: number = config.title.enabled ? 600 : 520;
    const bottomPadding: number = 40;

    const baseWidth: number = (poles.length * poleSpacing) +
        (config.careTips.enabled ? sidePadding + instructionsWidth : sidePadding);
    const totalHeight: number = nameTextY + bottomPadding;
    const instructionsX: number = sidePadding + (poles.length * poleSpacing) - 20;
    ...
    const poleStartX = config.careTips.enabled ? 150 : 50;
    ...
    ${poles.map((pole, index) => `
      <g transform="translate(${poleStartX + (index * poleSpacing)},${config.title.enabled ? '80' : '20'})">
